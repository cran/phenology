...
.Lnegbin <- function(x, pt) {
  ...
  if (windows) {
    clusterExport(cl = cl, 
                  varlist=c("datatot", "daily_count", "format_par", 
                            "pt", "xpar"), 
                  envir = environment())
    ...
    rg <- mclapply(X=seq_along(datatot), mc.cores = detectCores(), FUN = function(k) {
      # print(paste("pop", k))
      data <- datatot[[k]]
      # quel est le nom de la série en cours
      nmser <- names(datatot)[k]
...
.wrapperLnegbin <- function(...) {
  par <- unlist(list(...))
  return(getFromNamespace(".Lnegbin", ns = "phenology")(x=par["x"], pt=par[names(par) != "x"]))
}
