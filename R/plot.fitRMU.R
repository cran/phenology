...
#' @param at.legend Position of the legend
#' @param legend Text to show as legend. If FALSE, does not show legend. If NULL, it will use the column names for mean from RMU.names.
#' @description The function plot.fitRMU plots the results of fitRMU().\cr
#' The parameter at.legend can be bottomright, bottomleft, topleft or topright 
#' or a vector with c(x= , y= ).
...
plot.fitRMU <- 
	function(x, ..., what="proportions", at.legend="bottomright", legend=NULL) {
	  ...
	  if (what=="total") {
	    ...
	    year <- as.numeric(rownames(x[[what]]))
	    
	    pp <- modifyList(list(ylim=c(0,max(rowSums(x[[what]]))), xlab="Year",
	                        ylab=what, las=1, bty="n"), 
	                   list(...))
	  
	    pp <- modifyList(pp, list(x=year, y=x[[what]][,1], type="n"))
	  
	    do.call(plot, pp)

	    p <- cbind(rep(0, dim(x[[what]])[1]), rep(0, dim(x[[what]])[1]), x[[what]])
	    xx <- c(year, rev(year))
	    for (i in 2:(dim(p)[2]-1)) {
	      y <- c(apply(p[,1:i], 1, sum), rev(apply(p[,1:(i+1)], 1, sum)))
	      polygon(xx,y,col=rainbow(dim(x[[what]])[2])[i-1], border = NA)
	    }

	    if (all(legend!=FALSE)) {
	    
	      if (is.null(legend)) legend <- colnames(x[[what]])
	      
	    if (length(at.legend)==1) {
	      legend(at.legend, legend=rev(legend), col=rev(rainbow(dim(x[[what]])[2])), pch=15)
	    } else {
	      legend(x=at.legend["x"], y=at.legend["y"], legend=legend, 
	             col=rev(rainbow(dim(x[[what]])[2])), pch=15)
	    }
	    }
	  }
